...
7. **首页仪表盘控制（可选）**
   - `dashboard`：对象，用于控制首页仪表盘区域的组件显示
   - `dashboard.showWelcome`：布尔值，是否显示欢迎标题/副标题（默认 `true`）
   - `dashboard.showClock`：布尔值，是否显示实时时钟（默认 `true`）
   - `dashboard.showTodo`：布尔值，是否显示待办事项（默认 `true`）
   - 示例：
     ```yaml
     dashboard:
       showWelcome: true
